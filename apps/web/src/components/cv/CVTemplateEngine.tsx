...
export interface Variant { id: string; label: string; sidebar: string; accent: string; layout: string }
export interface SampleData { name: string; role: string; degree: string; school: string; company: string; period: string }
export interface PositionData { title: string; category: string; count: number; desc: string; sample: SampleData }

// ── Per-position variants ─────────────────────────────────────────────────────

export const POSITION_VARIANTS: Record<string, Variant[]> = {
  developer: [
...
export const lines = (widths: number[], h = 2.5, color = '#e5e7eb') =>
  widths.map((w, i) => (
  ...
  return (
    <div style={{ height: '100%', fontFamily: 'system-ui, sans-serif', background: '#fff', display: 'flex', flexDirection: 'column', overflowY: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0, borderBottom: '2px solid #f3f4f6' }}>
        {[['120%', 'Quota đạt được'], ['₫45 tỷ', 'Doanh số 2023'], ['350+', 'Khách hàng']].map(([val, label], i) => (
...
export function renderCV(layout: string, v: Variant, data: PositionData) {
  switch (layout) {
...
export function CVTemplatePreview({ variant, data }: { variant: Variant; data: PositionData }) {
...
export function CVTemplateThumbnail({
  variant, data, selected, onClick,
}: { variant: Variant; data: PositionData; selected: boolean; onClick: () => void }) {
  const W = 794, H = 1123, SCALE = 0.1
  return (
    <button
      onClick={onClick}
      className={`relative shrink-0 overflow-hidden rounded border-2 transition ${selected ? 'border-brand shadow-md shadow-brand/20' : 'border-gray-200 hover:border-gray-300'}`}
