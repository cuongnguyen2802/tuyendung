...
import {
  ...
  SearchIcon, InfoIcon, Loader2Icon, CheckIcon, XIcon, UploadCloudIcon,
} from 'lucide-react'
...
interface Article {
  ...
  imageUrl?: string; status: 'DRAFT' | 'PUBLISHED'; categoryId: string
...
type FormData = {
  ...
  categoryId: string; status: 'DRAFT' | 'PUBLISHED'
  metaTitle: string; metaDescription: string; metaKeywords: string
}
...
export default function EditArticlePage() {
  ...
  const { data: article, isLoading: loadingArticle } = useQuery<Article>({
    ...
    queryFn: () => api.get(`/articles/admin/${articleId}`),
    enabled: !!articleId,
  })
  ...
  const handleSubmit = async (publishNow?: boolean) => {
    ...
    try {
      const payload = { ...form, ...(publishNow !== undefined ? { status: publishNow ? 'PUBLISHED' : 'DRAFT' } : {}) }
      await api.put(`/articles/admin/${articleId}`, payload)
      if (publishNow !== undefined) setForm((f) => ({ ...f, status: publishNow ? 'PUBLISHED' : 'DRAFT' }))
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 line-clamp-1">{article?.title}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {/* Success toast */}
          {saved && (
            <span className="flex items-center gap-1.5 rounded-xl bg-brand-50 px-3 py-2 text-sm font-medium text-brand">
          ...
          {article?.status === 'PUBLISHED' && (
            <a
              href={`/blog/${article.category?.slug ?? 'career'}/${article.slug}`}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-5">
          ...
          {activeSection === 'content' && initialized && (
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            ...
            {form.imageUrl ? (
              <div className="group relative mb-3 overflow-hidden rounded-xl border border-gray-100">
                <img src={form.imageUrl} alt="Cover" className="h-40 w-full object-cover" />
